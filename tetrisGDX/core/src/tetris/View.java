        ...
        for (int r = 0; r < parameterObject.fs.length; r++) {
            for (int c = 0; c < parameterObject.fs[r].length; c++) {
                if (!parameterObject.drawBackground && parameterObject.fs[r][c] == 0)
                    continue;
                drawBox(r + parameterObject.row, c + parameterObject.col, BOX_SIZE, parameterObject.fs[r][c]);
            }
        }
    }

    private void drawBox(int col, int row, int size, int colorIndex) {
        graphics.drawBoxAt(ORIGIN + col * BOX_SIZE, ORIGIN + row * BOX_SIZE, colorIndex);
    }
